...
// DOC include start: DIFUZZRTL Bridge Driver Registeration
    #ifdef DIFUZZRTLBRIDGEMODULE_0_PRESENT
    DIFUZZRTLBRIDGEMODULE_0_substruct_create;

    #ifdef DIFUZZRTL
    difuzzrtl = new difuzzrtl_t(this, DIFUZZRTLBRIDGEMODULE_0_substruct);
    add_bridge_driver(difuzzrtl);
    #else
    add_bridge_driver(new difuzzrtl_t(this, DIFUZZRTLBRIDGEMODULE_0_substruct));
    #endif
    #endif
...
#ifdef DIFUZZRTL
void firesim_top_t::cov_init(int steps) {
    ...
    difuzzrtl->meta_reset(1);
    difuzzrtl->cov_init(1);
    step(steps, true);
    difuzzrtl->meta_reset(0);
    difuzzrtl->cov_init(0);
    ...
    fprintf(stderr, "Initialized coverage\n");
}

void firesim_top_t::bridge_reset(int onoff) {
    difuzzrtl->bridge_reset(onoff);
}

void firesim_top_t::meta_reset(int steps) {
    difuzzrtl->meta_reset(1);
    step(steps, true);
    difuzzrtl->meta_reset(0);
}

unsigned int firesim_top_t::get_covsum() {
    return difuzzrtl->read_covsum();
}
#endif
...
#ifdef DIFUZZRTL
    double sim_speed = ((double) end_cycle - last_cycle) / (sim_time * 1000.0);
    uint64_t tmp_cycle = end_cycle;
    end_cycle = end_cycle - last_cycle;
    last_cycle = tmp_cycle;
#else
