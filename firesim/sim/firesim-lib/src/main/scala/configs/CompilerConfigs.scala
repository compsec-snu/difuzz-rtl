...
import coverage.{regCoverage, bridgeReset}
...
class WithDifuzzRTLBridge extends Config((site, here, up) => {
  case TargetTransforms => Dependency[regCoverage] +: up(TargetTransforms, site)
  case HostTransforms => Dependency[bridgeReset] +: up(HostTransforms, site)
})

class DifuzzRTLF1Config extends Config(
  new WithDifuzzRTLBridge ++
  new WithAsyncResetReplacement ++
  new WithPlusArgReaderRemoval ++
  new WithEC2F1Artefacts ++
  new WithILATopWiringTransform ++
