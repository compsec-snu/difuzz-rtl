...
#ifdef DIFUZZRTL
int firesim_fesvr_t::host_fuzz_thread(void *arg)
{
    firesim_fesvr_t *fesvr = static_cast<firesim_fesvr_t*>(arg);

    // assert(!fesvr->elf.empty());
    fesvr->run(fesvr->elf);
...
// init is a blocking function which blocks until
// DifuzzRTL mutator prepares input executable file.
void firesim_fesvr_t::init()
{
    if (cov_init) return;
    
    this->reload();
    fz_scheduler->yield_control();
    elf = fz_scheduler->get_elf();
    printf("[FireSim] fesvr, elf: %s\n", elf.c_str());
    // elf = output + std::string(".input_0.elf");
    // elf = std::string("/home/centos/firesim/sim/output/f1/FireSim-DifuzzRTLRocketConfig-BaseF1Config/rv64ui-p-and");
    target = midas_context_t::current();
    is_busy = false;
    stopped = false;
    host.init(host_fuzz_thread, this);
}

void firesim_fesvr_t::finish()
{
    if (cov_init) {
        cov_init = false;
        return;
...
bool firesim_fesvr_t::is_cov_init()
{
    return cov_init;
}
#endif
...
#ifdef DIFUZZRTL
    fz_scheduler = new fuzz_scheduler_t(args);
    set_sigfile(fz_scheduler->get_sigfile());
...
void firesim_fesvr_t::idle()
{
    is_busy = false;
#ifdef DIFUZZRTL
    for (size_t i = 0 ; i < idle_counts && !this->timeout; i++) wait();
#else
