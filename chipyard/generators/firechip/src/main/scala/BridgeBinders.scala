...
import difuzzrtl.CanHaveDifuzzRTLImp
...
class WithDifuzzRTLBridge extends OverrideIOBinder({
  (system: CanHaveDifuzzRTLImp) =>
    system.difuzzrtl_io.foreach(u => DifuzzRTLBridge(system.clock, u)(system.p)); Nil
})
...
class WithDifuzzRTLFireSimBridges extends Config(
  new WithTiedOffSystemGPIO ++
  new WithTiedOffSystemDebug ++
  new WithTiedOffSystemInterrupts ++
  new WithSerialBridge ++
