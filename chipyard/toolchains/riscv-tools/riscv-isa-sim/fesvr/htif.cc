...
void htif_t::load_program()
{
  ...
  data_addrs.clear();
  for (int i = 0; i < 6; i++) {
    if (symbols.count("_random_data" + std::to_string(i)) && symbols.count("_end_data" + std::to_string(i))) {
        addr_t data_start = symbols["_random_data" + std::to_string(i)];
        addr_t data_end = symbols["_end_data" + std::to_string(i)];

        data_addrs.push_back(std::pair<addr_t, addr_t>(data_start, data_end));
      }
  }
...
void htif_t::set_sigfile(std::string sigfile) {
  sig_file = sigfile;
}
...
void htif_t::stop()
{
  if (!sig_file.empty() && sig_len) // print final torture test signature
  {
    ...
    for(auto iter = data_addrs.begin(); iter != data_addrs.end(); iter++) {
      addr_t data_start = iter->first;
      addr_t data_len = iter->second - iter->first;
      std::vector<uint8_t> data_buf(data_len);
      mem.read(data_start, data_len, &buf[0]);

      assert(data_len % incr == 0);
      for (addr_t i = 0; i < data_len; i += incr) {
        for (addr_t j = incr; j > 0; j--)
          sigs << std::setw(2) << (uint16_t)buf[i+j-1];
        sigs << '\n';
      }
...
int htif_t::run(std::string elf)
{
  ...
  while (!signal_exit && exitcode == 0 && !timeout)
  {
