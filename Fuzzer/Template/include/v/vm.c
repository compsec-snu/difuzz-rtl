...
void handle_fault(uintptr_t addr, uintptr_t cause)
{
  ...
  uintptr_t new_pte = 0;
  if (random == 0) {
    new_pte = (node->addr >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_U | PTE_R | PTE_W | PTE_X;
  } else {
    new_pte = (node->addr >> PGSHIFT << PTE_PPN_SHIFT) | ((random >> 16) & 0xff);
  }
...
void handle_trap(trapframe_t* tf)
{
  ...
  //   terminate(1);
  // }
  // if (tf->cause == CAUSE_ILLEGAL_INSTRUCTION)
  // {
  //   assert(tf->epc % 4 == 0);
