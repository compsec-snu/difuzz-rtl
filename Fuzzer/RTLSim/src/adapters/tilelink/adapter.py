...
class tlAdapter():
    ...
    def probe_blocks_rec(self, probe_addrs, b_srcs, b_callback):
        ...
        if probe_addrs:
            ...
            self.b_queue.push('ProbeBlock', param=toN, size=size, \
    ...
    def drive_input(self, memory):
        ...
        b_src_list = [i for i in range(0, 3)] # TODO, RocketTile only has 1 b_src
        ...
        self.host_if = cocotb.fork(self.host_interface(block_perm, b_srcs))
            ...
            if a_ports.fire():
                ...
                if opcode == GET:
                    ...
                    if block_perm[block_addr] != TIP:
                        b_src = b_srcs.get()
                        self.b_queue.push('ProbeBlock', param=toT, size=size, \
                                          source=b_src, address=addr, mask=mask)
                ...
                if opcode == ACQUIRE_BLOCK and \
                   self.protocol == TL_C:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        self.b_queue.push('ProbeBlock', param=b_param, size=size, \
                                          source=b_src, address=addr, mask=mask)
                ...
                if opcode == ACQUIRE_PERM and \
                   self.protocol == TL_C:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        self.b_queue.push('ProbePerm', param=b_param, size=size, \
                                          source=b_src, address=addr, mask=mask)
    ...
    @coroutine
    def host_interface(self, block_perm, b_srcs):
        clkedge = RisingEdge(self.dut.clock)

        while self.drive:
            if self.probe and self.probe_addr not in self.ongoing_tlc.values():
                ...
                if block_perm[block_addr] != TIP:
                    b_src = b_srcs.get()
                    self.b_queue.push('ProbeBlock', param=toN, size=size, \
                                      source=b_src, address=self.probe_addr, mask=mask)
