...
static void help(int exit_code = 1)
{
  ...
  fprintf(stderr, "  -intr=<path>          Pseudo interrupt file specifying the pc to assert interrupt\n");
...
static std::map<reg_t, reg_t> read_intr(const char *filename, reg_t fileoff)
{
  std::ifstream in(filename, std::ios::in);
  std::string line;

  std::map<reg_t, reg_t> intrs;

  char *p;
  while(std::getline(in, line)) {
    const char *pair = line.c_str();
    auto pc = strtoull(pair, &p, 16);
    if (!*p || *p != ':')
      help();

    auto intr = strtoull(p+1, &p, 2);
    if (*p)
      help();
    ...
    intrs.insert(std::make_pair(pc, intr));
  }
...
int main(int argc, char** argv)
{
  ...
  parser.option(0, "intr", 1, [&](const char* s){intr = s;});
  ...
  if (intr && check_file_exists(intr)) {
    intr_size = get_file_size(intr);
    intrs = read_intr(intr, intr_offset);
  }
  ...
  sim_t s(isa, priv, varch, nprocs, intrs, halted, real_time_clint,
