...
void processor_t::take_trap(trap_t& t, reg_t epc)
{
  ...
  if (state.prv <= PRV_S && bit < max_xlen && ((vsdeleg >> bit) & 1)) {
    ...
    // Match Rocket CSR
    reg_t tval = t.get_tval();
    reg_t upper_tval = (tval >> 39) & 0x1ffffff;
    if (((tval >> 38) & 0x1) && (upper_tval == 0x1ffffff))
      tval = tval | (reg_t(1) << 39);
    else if (!((tval >> 38) & 0x1) && (upper_tval != 0x0))
      tval = tval | (reg_t(1) << 39);
    else
      tval = tval & 0xffffff7fffffffff;

    reg_t upper_mask = (tval >> 39) & 0x1? 0xffffff : 0x0;
    state.mtval = (upper_mask << 40) | (tval & 0xffffffffff);
...
#define LOG_CSR(rd)
  ...
  switch (which)
  {
    ...
    case CSR_MSTATUS: {
      ...
      // Match Rocket CSR
      // 1. Set FS bits only either OFF (2b'00) or DIRTY (2b'11)
      reg_t rocket_mask = (val & MSTATUS_FS) ? MSTATUS_FS : 0;
      ...
      state.mstatus = (state.mstatus & ~mask) | (val & mask | rocket_mask);
    ...
    case CSR_MIP: {
      // Match Rocket CSR
      reg_t mask = (supervisor_ints | hypervisor_ints) & (MIP_SSIP | MIP_STIP | vssip_int | MIP_SEIP | MIP_MEIP);
    ...
    case CSR_SATP: {
      ...
      if (max_xlen == 64 && (get_field(val, SATP64_MODE) == SATP_MODE_OFF ||
        ...
        reg_val = val & ((SATP64_PPN & rv64_ppn_mask) | SATP64_MODE);
      if (state.v)
        state.vsatp = reg_val;
      else
        state.satp = reg_val;
    ...
    case CSR_SEPC:
      if (state.v)
        ...
        reg_t mask = (reg_t(1) << 40) - 1;
        reg_t upper_mask = -((val >> 39) & 1) << 39;
        state.sepc = upper_mask | ((val & ~(reg_t)1) & mask);
      }
      break;
    case CSR_STVEC:
      if (state.v)
    ...
    case CSR_SCAUSE:
      if (state.v)
        ...
        // Match Rocket CSR
        reg_t mask = (reg_t(1) << (xlen -1)) + 31;
        state.scause = (state.scause & ~mask) | (val & mask);
      }
        // state.scause = val;
      break;
    case CSR_STVAL:
    ...
    case CSR_MEPC: {
      reg_t mask = (reg_t(1) << 40) - 1;
      reg_t upper_mask = -((val >> 39) & 1) << 39;
      state.mepc = upper_mask | ((val & ~(reg_t) 1) & mask);
      break;
    }
    case CSR_MTVEC: state.mtvec = val & ~(reg_t)2; break;
    case CSR_MSCRATCH: state.mscratch = val; break;
    case CSR_MCAUSE: {
      reg_t mask = (reg_t(1) << (xlen - 1)) + 15;
        (1 << CAUSE_MISALIGNED_FETCH) |
        ...
        // (1 << CAUSE_VIRTUAL_INSTRUCTION) |
        // (1 << CAUSE_STORE_GUEST_PAGE_FAULT);
      state.mcause = (state.mcause & ~mask) | (val & mask);
      break;
    }
